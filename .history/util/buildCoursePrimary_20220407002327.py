...
def buildCoursePrimary(initial_data):
    ...
    test_review = {
        "text": hasattr(["text"] or "",
        "likes": hasattr(["likes"] or 0,
        "dislikes": hasattr(["dislikes"] or 0,
        "will_take_again": hasattr(["will_take_again"] or "",
        "professor": hasattr(["professor"] or "",
        "semester": hasattr(["semester"] or "",
        "name": hasattr(["name"] or "",
    }
        ...
        for dept, courses in all_descriptions.items():
            for crn, metadata in courses.items():
                course_and_review = {
                    ...
                    "department": dept,
                    "reviews": [test_review] if hasattr(["text"] else [],
                    "number_of_reviews": hasattr(["number_of_reviews"] or 0,
                    "sum_of_difficulty": hasattr(["sum_of_difficulty"] or 0,
                    "sum_of_organization": hasattr(["sum_of_organization"]
                    or 0,
                    "sum_of_overall_quality": hasattr([
                        "sum_of_overall_quality"
                    ]
                    or 0,
                    "sum_of_will_take_again": hasattr([
