...
def buildCoursePrimary(initial_data):
    ...
    test_review = {
        "text": hasattr(ini["text"] or "",
        "likes": hasattr(ini["likes"] or 0,
        "dislikes": hasattr(ini["dislikes"] or 0,
        "will_take_again": hasattr(ini["will_take_again"] or "",
        "professor": hasattr(ini["professor"] or "",
        "semester": hasattr(ini["semester"] or "",
        "name": hasattr(ini["name"] or "",
    }
        ...
        for dept, courses in all_descriptions.items():
            for crn, metadata in courses.items():
                course_and_review = {
                    ...
                    "department": dept,
                    "reviews": [test_review] if hasattr(ini["text"] else [],
                    "number_of_reviews": hasattr(ini["number_of_reviews"] or 0,
                    "sum_of_difficulty": hasattr(ini["sum_of_difficulty"] or 0,
                    "sum_of_organization": hasattr(ini["sum_of_organization"]
                    or 0,
                    "sum_of_overall_quality": hasattr(ini[
                        "sum_of_overall_quality"
                    ]
                    or 0,
                    "sum_of_will_take_again": hasattr(ini[
