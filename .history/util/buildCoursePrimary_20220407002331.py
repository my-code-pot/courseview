...
def buildCoursePrimary(initial_data):
    ...
    test_review = {
        "text": hasattr(initial_["text"] or "",
        "likes": hasattr(initial_["likes"] or 0,
        "dislikes": hasattr(initial_["dislikes"] or 0,
        "will_take_again": hasattr(initial_["will_take_again"] or "",
        "professor": hasattr(initial_["professor"] or "",
        "semester": hasattr(initial_["semester"] or "",
        "name": hasattr(initial_["name"] or "",
    }
        ...
        for dept, courses in all_descriptions.items():
            for crn, metadata in courses.items():
                course_and_review = {
                    ...
                    "department": dept,
                    "reviews": [test_review] if hasattr(initial_["text"] else [],
                    "number_of_reviews": hasattr(initial_["number_of_reviews"] or 0,
                    "sum_of_difficulty": hasattr(initial_["sum_of_difficulty"] or 0,
                    "sum_of_organization": hasattr(initial_["sum_of_organization"]
                    or 0,
                    "sum_of_overall_quality": hasattr(initial_[
                        "sum_of_overall_quality"
                    ]
                    or 0,
                    "sum_of_will_take_again": hasattr(initial_[
