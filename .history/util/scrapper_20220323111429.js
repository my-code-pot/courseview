...
async function exampleAll() {
  ...
  try {
    ...
    for (let i = 0; i < dept_nums.length; i++) {
      const a = await dept_nums[i].findElement(By.tagName("a")).click();
      const course = await driver.wait(until.elementLocated(By.tagName(`td`)));
      const div = await course.findElements(
        By.className("gatewaye-mini-toolbar")
      );
      const descp = await div[3].getText();
      console.log(descp);
      console.log("");
      if (i > 0) {
        break;
