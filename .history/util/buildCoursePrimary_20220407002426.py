...
def buildCoursePrimary(initial_data):
    ...
    # courses_and_reviews.delete_many({})  # removes all the existing documents

    test_review = {
        "text": hasattr(initial_data, "text") or "",
        "likes": hasattr(initial_data, "likes") or 0,
        "dislikes": hasattr(initial_data, "dislikes") or 0,
        "will_take_again": hasattr(initial_data, "will_take_again") or "",
        "professor": hasattr(initial_data, "professor") or "",
        "semester": hasattr(initial_data, "semester") or "",
        "name": hasattr(initial_data, "name") or "",
    }
        ...
        for dept, courses in all_descriptions.items():
            for crn, metadata in courses.items():
                course_and_review = {
                    ...
                    "department": dept,
                    "reviews": [test_review] if hasattr(initial_data, "text"] else [],
                    "number_of_reviews": hasattr(initial_data, "number_of_reviews"] or 0,
                    "sum_of_difficulty": hasattr(initial_data, "sum_of_difficulty"] or 0,
                    "sum_of_organization": hasattr(initial_data, "sum_of_organization"]
                    or 0,
                    "sum_of_overall_quality": hasattr(initial_data, 
                        "sum_of_overall_quality"
                    ]
                    or 0,
                    "sum_of_will_take_again": hasattr(initial_data, 
...
if __name__ == "__main__":
    ...
    initial_data = {
        "text": """This course is a great way to get started in the field of computer science.
