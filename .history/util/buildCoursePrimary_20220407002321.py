...
def buildCoursePrimary(initial_data):
    ...
    test_review = {
        "text": has["text"] or "",
        "likes": has["likes"] or 0,
        "dislikes": has["dislikes"] or 0,
        "will_take_again": has["will_take_again"] or "",
        "professor": has["professor"] or "",
        "semester": has["semester"] or "",
        "name": has["name"] or "",
    }
        ...
        for dept, courses in all_descriptions.items():
            for crn, metadata in courses.items():
                course_and_review = {
                    ...
                    "department": dept,
                    "reviews": [test_review] if has["text"] else [],
                    "number_of_reviews": has["number_of_reviews"] or 0,
                    "sum_of_difficulty": has["sum_of_difficulty"] or 0,
                    "sum_of_organization": has["sum_of_organization"]
                    or 0,
                    "sum_of_overall_quality": has[
                        "sum_of_overall_quality"
                    ]
                    or 0,
                    "sum_of_will_take_again": has[
