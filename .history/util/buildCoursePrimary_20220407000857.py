    ...
    with open("description.json", "r") as f:
        ...
        for dept, courses in all_descriptions.items():
            for crn, metadata in courses.items():
                course_and_review = {"crn": crn, "title": metadata["title"], "name": metadata["name"], "description": metadata["description"], "department": dept, "reviews": [test_review] if initial_data["text"] else [], "number_of_reviews": initial_data["number_of_reviews"] or 0, "sum_of_difficulty": initial_data["sum_of_difficulty"] or 0, "sum_of_overall_quality": initial_data["sum_of_overall_quality"] or 0, "sum_of_will_take_again": initial_data["sum_of_will_take_again"] or 0}
