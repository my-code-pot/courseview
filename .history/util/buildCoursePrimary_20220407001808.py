...
def buildCoursePrimary(initial_data):
    courses_and_reviews_test = db.courses_and_reviews_test
    print(courses_and_reviews_test.count_documents({}))
    courses_and_reviews_test.delete_many({})  # removes all the existing documents
        ...
        for dept, courses in all_descriptions.items():
            for crn, metadata in courses.items():
                course_and_review = {
                    ...
                    "sum_of_organization": initial_data["sum_of_organization"]
                    or 0,
                ...
                courses_and_reviews_test.insert_one(course_and_review)
    print(courses_and_reviews_test.count_documents({}))
